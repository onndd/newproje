...
def optimize_mlp(X, y, n_trials=20, scoring_params=None, timeout=600):
    ...
    def objective(trial):
        # Oversampling Ratio Optimization
        os_ratio = trial.suggest_float('os_ratio', 1.0, 3.0)
        
        # Apply Oversampling (Manual implementation or SMOTE)
        # Here we use simple duplication of minority class 0
        # Determine minority class (usually 0: don't bet)
        # But wait, class 0 (wait) is currently minority in predictions? 
        # Actually in data, 1.5x+ is roughly 30-40%. So 0 is majority usually.
        # But user wants to weight class 0 more?
        # Let's assume balance_data handles it based on target_class=0 (Negatives)
        
        from .model_mlp import balance_data
        X_t_balanced, y_t_balanced = balance_data(X_train, y_train, target_class=0, multiplier=os_ratio)
...
def optimize_lstm(X, y, n_trials=10, scoring_params=None, timeout=600):
    ...
    # Note: LSTM optimization can be slow.
    # X here is expected to be 'values' array for sequence generation
    # OR X_train dataframe.
    # Based on recent edits, we support both but prefer sequence logic internally.
    
    # Assume X is values array if y is provided as secondary arg, OR handle DataFrame
    # Ideally standalone_runner passes raw values.
    
    # To keep it simple and consistent with previous fix:
    # We will assume X is 'values' Series/Array for optimization
    if isinstance(X, pd.DataFrame):
        values = X['values'].values
    else:
        values = X
        
    # Split raw values (chronological)
    split = int(len(values) * 0.8)
    train_values = values[:split]
    val_values = values[split:]
    
    # We need to generate targets for validation to score
    # This part is tricky inside objective without full pipeline.
    # Re-using internal generation logic.
    
    from .model_lstm import create_sequences
    SEQ_LEN = 60
    
    # Pre-compute Validation Data (Fixed) to save time
    # NO! Data Leakage Risk if we accept full dataset.
    # Only use val_values.
    
    # Create Scaler on TRAIN only
    from sklearn.preprocessing import MinMaxScaler
    scaler = MinMaxScaler()
    train_normalized = scaler.fit_transform(train_values.reshape(-1, 1))
    val_normalized = scaler.transform(val_values.reshape(-1, 1))
    
    # Generate Sequences
    def get_data(seq_len):
        X_t, y_t = create_sequences(train_normalized, seq_len, 1.50) # Assuming P1.5 target for opt
        X_v, y_v = create_sequences(val_normalized, seq_len, 1.50)
    ...
    def objective(trial):
        ...
        # Determine target (hacky for generic 'y' arg)
        # If y is passed, check if it looks like P3
        # For now optimize for P1.5 structure mainly
        
        X_t, y_t, X_v, y_v = get_data(SEQ_LEN)
        ...
        try:
            ...
            # Use Focal Loss if possible or binary_crossentropy
    ...
    # Return Best Params + Params dict structure
    # Pack it so standalone runner can unpack
